...
export function DreAssistentePanel({ lancamentos }: DreAssistentePanelProps) {
  ...
  const analisarDre = async () => {
    ...
    if (lancamentos.length === 0) {
      setError('Adicione lançamentos antes de solicitar a análise.')
      return
    }
    ...
    try {
      const res = await fetch('/api/assistente-dre', {
        ...
        body: JSON.stringify({
          dre: serializeLancamentos(lancamentos),
  ...
  return (
    <section className={styles.panel}>
      <div className={styles.header}>
        ...
        <p>
          A análise usa automaticamente os lançamentos já cadastrados na tabela abaixo.
          Se quiser, adicione uma observação extra para orientar o assistente.
        </p>
      </div>
      ...
      <textarea
        ...
        placeholder="Opcional: ex. analisar foco em margem, custos fixos e possíveis reclassificações"
      />

      <button className={styles.button} onClick={analisarDre} disabled={loading}>
        {loading ? 'Analisando...' : 'Analisar lançamentos do DRE'}
      </button>
