...
const normalizeDreInput = (dre: unknown): string => {
  if (typeof dre === 'string') return dre.trim()
  if (Array.isArray(dre) || (dre && typeof dre === 'object')) {
    return JSON.stringify(dre, null, 2)
  }
  return ''
}

const buildMessages = (dreInput: string, context: string, observacao?: string): ChatMessage[] => {
  const userContent = observacao
    ? `DRE enviado pelo usuário:\n\n${dreInput}\n\nObservação adicional do usuário:\n${observacao}`
    : `DRE enviado pelo usuário:\n\n${dreInput}`
...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    const dreInput = normalizeDreInput(body?.dre ?? body?.input)
    const observacao = typeof body?.observacao === 'string' ? body.observacao.trim() : ''

    if (!dreInput) {
      return res.status(400).json({ error: 'Envie o DRE em `dre` (texto, JSON ou array de lançamentos).' })
    }

    const contextResult = await loadDreContext()
    const messages = buildMessages(dreInput, contextResult.context, observacao || undefined)
